...
public class Adventure {
            ...
            for (int i = 0; i < map.length; i++) {
                ...
                for (int j = 0; j < map[i].length; j++) {
                    ...
                    writer.print("[  " + "  ]");
                        ...
                        writer.print("[");
                       String fieldName = map[i][j][Config.NAME];
    ...
    public static int[] parseCoordinates(String coordinateString) {
        try {
            if ( coordinateString == null || coordinateString.length () == 0 ) {
                return null;
            } else if ( !coordinateString.contains (",") ) {
                return null;
            } else {

                String[] parseCoordinate=coordinateString.split(",");

                int[] returnArray=new int[2];


                for (int i = 0; i < parseCoordinate.length; i++) {
                    parseCoordinate[i]=parseCoordinate[i].trim();

                }

                if ( parseCoordinate.length == 0 || parseCoordinate.length == 1 ) {
                    return null;
                }
                boolean isCoordinate = true;
                String first = parseCoordinate[0];
                String second = parseCoordinate[1];
                ...
                if ( isCoordinate ) {
                    returnArray[0] = Integer.parseInt(first);
                    returnArray[1] = Integer.parseInt(second);
                    return returnArray;
                } else {
                    return null;
                }
    ...
    public static int[] determineLocation(String[][][] map, int[] currentLocation, String direction) {
        ...
        if ( direction.equals(Config.MOVE_DOWN) ) {
            if ( currentLocation[0] == map.length - 1 ) {
                newLocation[0] = 0;
                newLocation[1] = currentLocation[1];
            } else {
                newLocation[0] = currentLocation[0] + 1;
        ...
        if ( direction.equals(Config.MOVE_LEFT) ) {
            if ( currentLocation[1] == 0 ) {
                newLocation[1] = map[0].length - 1;
        ...
        if ( direction.equals (Config.MOVE_RIGHT) ) {
            if ( currentLocation[1] == map[0].length - 1 ) {
                newLocation[1] = 0;
                newLocation[0] = currentLocation[0];
            } else {
                newLocation[1] = currentLocation[1] + 1;
    ...
    public static boolean enter(String[][][] map, int[] location, Random rand) {
        ...
        if ( fields == null ) {
            ...
            if ( fields[Config.ENTER_RESULT].equals (Config.RESULT_WIN) ||
